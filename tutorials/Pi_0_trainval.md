...
- 如果发生没有办法链接到hf，如下所示
```bash
'(ProtoolBror('Conection aborted.', ConnectionRsetEror(104, Conection reset by peer')), '(Request ID: 17fec5c3-elee-42f2-bd8b-a24def2a7857)')' thrown while requesting EAD htps
/huggingface.co/google/paligemma-3b-pt-224/resolve/main/config.json
Retrying in 8s [Retry 5/5].
```
- 手动下载paligemma-3b-pt-224，并在pi0_base中替换路径
```bash
modelscope download --model google/paligemma-3b-pt-224 --local_dir /workspace/models/paligemma-3b-pt-224
vim /workspace/models/pi0_base/policy_preprocessor.json
#将"tokenizer_name": "google/paligemma-3b-pt-224"修改为本地路径，例如/workspace/models/paligemma-3b-pt-224
```
- 如果发生训练完成后卡住，长时间无法结束，强制中断后，wandb上记录了所有步数，但是显示失败了，可以关闭wandb，或者离线再sync
