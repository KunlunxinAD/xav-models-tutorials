...
cd /home
git lfs install
git clone https://www.modelscope.cn/maple77/Qwen2-7B-Instruct.git

# 下载LLaMA-Factory代码以及配置
git clone https://github.com/hiyouga/LLaMA-Factory.git

# 下载qwen2-7b 代码以及配置
wget https://klx-sdk-release-public.su.bcebos.com/v1/xav/code/qwen2_7b.tar.gz
```

## 启动容器
```bash
export XAV_IMAGE=<XAV_IMAGE>
export LLAMA_CONTAINER=Qwen2_7B_test
export MODEL_PATH=</path/to/qwen2_7b> #本地路径
...
```bash
# 创建conda env
conda create -n Qwen2_7b_env --clone python310_torch25_cuda
conda init bash
conda activate Qwen2_7b_env
# 拉取xmlir 产出
...
vim qwen2_7b_full_sft.yaml
vim qwen2_7b_lora_sft.yaml
```

### 执行Lora
```bash
source env.sh
bash train_qwen2_lora.sh 
```

### 执行sft
```bash
source env.sh
bash train_qwen2_sft.sh
```